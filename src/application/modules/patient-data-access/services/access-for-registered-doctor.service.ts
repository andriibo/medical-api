...
export class AccessForRegisteredDoctorService {
    ...
    public async initiateDataAccess(patient: User, doctor: User): Promise<void> {
        await this.patientDataAccessSpecification.assertPatientCanGiveAccessForUser(patient, doctor);

        const dataAccess = this.createDataAccess(patient, doctor);

        await this.patientDataAccessRepository.create(dataAccess);

        await this.patientDataAccessEventEmitter.emitPatientInitiatedAccessForRegisteredDoctor(patient, doctor.email);
    }
