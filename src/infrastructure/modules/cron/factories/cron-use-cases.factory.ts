import {Inject, Injectable} from '@nestjs/common';
import {RemoveUsersUseCase} from 'app/modules/profile/use-cases/profile';
import {IUserRepository} from 'app/modules/auth/repositories';
import {IRemoveUserService} from 'app/modules/profile/services/remove-user.service';
import {RemoveCaregiverOrPatientService} from 'infrastructure/modules/profile/services/remove-caregiver-or-patient.service';
import {RemoveDoctorService} from 'infrastructure/modules/profile/services/remove-doctor.service';

@Injectable()
export class CronUseCasesFactory {
    public constructor(
        @Inject(IUserRepository) private readonly userRepository: IUserRepository,
        @Inject(IRemoveUserService)
        private readonly removeCaregiverOrPatientService: RemoveCaregiverOrPatientService,
        @Inject(IRemoveUserService) private readonly removeDoctorService: RemoveDoctorService,
    ) {}

    public createRemoveUsersUseCase(): RemoveUsersUseCase {
        return new RemoveUsersUseCase(
            this.userRepository,
            this.removeCaregiverOrPatientService,
            this.removeDoctorService,
        );
    }
}
